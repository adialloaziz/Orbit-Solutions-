...
from utility import orbit, Mckean_Vlasov, BrusselatorModel, optim_BrusselatorModel,call_method
...
def run(model,n_z,orbit_method,p0,filename=None):
    ...
    t_span = (0, 10*model.T_ini)

    #We integrate sufficiently the equation to find a good starting point
    phi_t = sol = solve_ivp(f, t_span, y0, method='BDF', jac = model.jacobian,
                     rtol=1e-7, atol=1e-9,
    ...
    start = time.time()
    k, T_by_iter, y_by_iter, Norm_B, Abs_Err, Rel_Err, converged, integral_values = call_method(method_to_call, **args_func)
    end = time.time()
...
#_____Handling command line arguments_____
    ...
    res = run(model,args.n_z,args.method,args.p0,filename_prof)
    #Saving the results
