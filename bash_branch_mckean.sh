...
mkdir -p logs/branch_mckean_vlasov
mkdir -p jobs/branch_mckean_vlasov
#source $HOME/.bashrc
...
cat > jobs/branch_mckean_vlasov/job_branch_mckean_vlasov.sh <<EOF
...
#PBS -q pmedium
cd $DIR
$DIR/.myvenv/bin/python3 $DIR/test_Mckean_Vlasov.py
...
qsub jobs/branch_mckean_vlasov/job_branch_mckean_vlasov.sh
echo "Submitted job script: branch_mckean_vlasov/job_branch_mckean_vlasov.sh"	
