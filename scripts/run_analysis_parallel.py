...
from scripts.models import BrusselatorModel, optim_BrusselatorModel
...
def run(model,n_z,orbit_method):
    ...
    output_file = f"{Dir_path}/{orbit_method}_results_{n_z}.txt"
...
#define a function for the argsparser to handle the command line arguments
def prog_options():
    ...
    parser.add_argument(
                    "-sparse_jac","--sparse_jac", type=int, default=0,choices=[0,1],
                    help="""If True, use sparse jacobian matrix. Default is 0 (dense jacobian)"""

                      )
    
    parser.add_argument(
                    "-method", "--method",type=str, nargs='?', default="Newton_orbit", choices=["Newton_orbit", "Newton_Picard_sub_proj"],
                    help="""The method to use for the orbit finding. Default is 'Newton_orbit'.
                            Other options include 'Newton_Picard_subsp_iter'."""
                      )
    parser.add_argument(
                    "-output_dir", "--output_dir", type=str, default=None, 
                    help="""The directory where the results will be saved.
                            If not provided, a default file will be created with the same name as the 
                            params file in the "./Results" directory.
                            Please provide a only the name of the directory, not the full path. 
                            Useful if the inputs in the batch were not all processed.
                          ."""
...
if __name__ == "__main__":
    ...
    if args.output_dir is None:
        ...
        Dir_path = Path(output_root_dir/args.param_file/args.output_dir)
        Dir_path.mkdir(parents=True, exist_ok=True)
    ...
    if not remaining_inputs:
        print("All inputs have already been processed.")
        exit(0)

    #testing available cores
    N_cores = args.k_dim
    if args.k_dim > cpu_count():
        print(f"[WARNING] The number of cores requested ({args.k_dim}) exceeds the available cores ({cpu_count()}). Using {cpu_count()} cores instead.")
        N_cores = cpu_count()
    else:
   ...
   # all_results = []
    for i in range(0, len(remaining_inputs), batch_size):
        ...
        res = Parallel(n_jobs=N_cores,backend="multiprocessing",
